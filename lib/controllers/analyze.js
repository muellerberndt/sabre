...
module.exports = async (env, args) => {
    ...
    try {
        ...
        const resolvedSources = (await Profiler.required_sources(config.with({
            paths: [solidityFilePath],
            resolver: resolver,
            base_path: solidityFileDir,
            contracts_directory: solidityFilePath
          }))).allSources;
        ...
        Object.keys(resolvedSources).forEach((file) => {
            allSources[file] = { content: resolvedSources[file] };
        })
        ...
        const mxClient = client.initialize(apiUrl, apiKey);
        ...
        const { uuid } = await client.submitDataForAnalysis(mxClient, data, args.isCheckProperty);
        ...
        if (uniqueIssues.length === 0) {
            ...
            // Custom text format for property checking
            const format = args.isCheckProperty && args.format === 'text' ? 'propertyCheckText' : args.format;
            const formatter = report.getFormatter(format);
