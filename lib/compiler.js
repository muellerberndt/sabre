...
/* solc-js of specific version from temp directory, if exists */
...
const getFileContent = (filepath) => {
    ...
    let npmPath = process.cwd() + '/node_modules/' + filepath;
    const npmStats = fs.statSync(npmPath)
    

    if (stats.isFile()) {
        importedFiles.push(filepath)
        return fs.readFileSync(filepath).toString()
    } else if (npmStats.isFile()) {
        importedFiles.push(npmPath)
        return fs.readFileSync(npmPath).toString()
    } else {        
...
const getCompiledContracts = (input, solcSnapshot, solidity_file_name, compile_contract_name) => {
    const compiled = JSON.parse(solcSnapshot.compile(JSON.stringify(input)), findImports);
    ...
    if (inputfile.length === 0) {
        ...
        if (compile_contract_name && inputfile[compile_contract_name]) {
            contractName = compile_contract_name;
            contract = inputfile[compile_contract_name];
        } else {
            /**
