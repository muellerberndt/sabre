...
textFormatter.formatTestCaseSteps = (steps, fnHashes) => {
    ...
    for (let s = 0, n = 0; s < steps.length; s++) {
        ...
        const fnName = step.name || fnHashes[fnHash] || '<N/A>';
...
textFormatter.formatTestCase = (testCase, fnHashes) => {
    ...
    if (testCase.steps) {
        output.push(
            ...
            '',
            textFormatter.formatTestCaseSteps(testCase.steps, fnHashes)
        );
    }
...
textFormatter.formatMessage = (message, filePath, sourceCode, fnHashes) => {
    ...
    if (mythxIssue.locations.length) {
        ...
        if (src) {
            code = textFormatter.getCodeSample(sourceCode, src);
        }
    }
    ...
    if (testCases) {
        for (const testCase of testCases) {
            const section = textFormatter.formatTestCase(testCase, fnHashes);
